...
@Service
public class CurrentReadingListner implements HoldingRegisterListner {

    private final Logger logger = LoggerFactory.getLogger(CurrentReadingListner.class);
    ...
    public void onHoldingRegisterReceived(int[] registerValues) {
        ...
        logger.info("register value of current : " + registerValues[config.registerid]);
        double val = registerValues[config.registerid];
        //double eng_unit = ((val / 65535) * 5);
        //logger.info("current value "+eng_unit + " actual value "+val);
        String result = "";
        try {
        ...
        logger.info("current value " + result + " actual value " + val);
        sensorReadingModel.addReading(config.sensorkey, result);
        sensorReadingModel.key = config.sensorkey;
        logger.info("publishing current readings to cloud connector..");

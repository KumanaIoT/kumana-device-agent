...
@Service
public class LuxReadingListner implements HoldingRegisterListner {

    private final Logger logger = LoggerFactory.getLogger(LuxReadingListner.class);
    ...
    public void onHoldingRegisterReceived(int[] registerValues) {
        ...
        //double resistorVoltage = ((val / 65535) * 5);
        ...
        logger.info("lux value " + result + " actual value " + val);
        sensorReadingModel.addReading(config.sensorkey, result);
        sensorReadingModel.key = config.sensorkey;
        logger.info("publishing lux readings to cloud connector..");

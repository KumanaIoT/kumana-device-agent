...
@Service
public class BatteryReadingListner implements HoldingRegisterListner {

    private final Logger logger = LoggerFactory.getLogger(BatteryReadingListner.class);
    ...
    public void onHoldingRegisterReceived(int[] registerValues) {
        ...
        logger.info("battery value " + result + " actual value " + val);
        sensorReadingModel.addReading(config.sensorkey, result);
        sensorReadingModel.key = config.sensorkey;
        logger.info("publishing battery readings to cloud connector..");

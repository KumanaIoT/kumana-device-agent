...
@Service
public class WaterlvlReadingListner implements HoldingRegisterListner {

    private final Logger logger = LoggerFactory.getLogger(WaterlvlReadingListner.class);


    @Autowired
    WaterLvlConfig config;
    ...
    public void onHoldingRegisterReceived(int[] registerValues) {
        ...
        logger.info("waterlvl value " + result + " actual value " + val);
        sensorReadingModel.addReading(config.sensorkey, result);
        sensorReadingModel.key = config.sensorkey;
        logger.info("publishing waterlvl readings to cloud connector..");
